---
title: "설정 파일"
permalink: /blog/configuration/
excerpt: "config 파일을 수정하는 방법"
...
Github blog의 모든 세팅이 포함되어 있는 파일은 `_config.yml` 로, [Jekyll](https://jekyllrb.com/docs/configuration/) 의 파일구조에서 root에 기본적으로 존재하는 파일입니다.

아래 설명은 `_config.yml`의 내용을 다루며, (아마도) config 파일에 적혀있는 순서되로 설명하고 있을겁니다.

**Note:** 기술적인 문제로 인해  `_config.yml` 는 local에서 `jekyll serve` 명령어로는 갱신되지 않기 때문에 변경점이 생겼다면 서버를 재시작하여야지만 적용됩니다. GITHUB에 올린 경우 일정 시간이 지나야지만 (수 시간 정도) 반영이 됩니다.
{: .notice--warning}
...
로컬에서 블로그를 구동하거나, Ruby gem 버젼을 사용할 경우 theme 부분을 아래와 같이 활성화 해주어야 합니다.
...
스킨을 여러 개 중 하나로 바꿀 수 있는데, 이것 또한 로컬로 테마를 사용할 떄만 적용되는 듯 합니다.

```yaml
minimal_mistakes_skin: "default" # "air", "aqua", "contrast", "dark", "dirt", "neon", "mint", "plum", "sunrise"
```

여러 스킨의 종류가 있는데, 자세한 사항은 [minimal mistakes 홈페이지](https://mmistakes.github.io/minimal-mistakes/docs/configuration/#skin)를 참고 부탁드립니다.

### 언어 설정

`site.locale` 은 해당 사이트의 언어를 결정합니다.

_Example:_ `locale: "en-US"` 는 `lang` 어트리뷰트를 _미국식_ 영어로 수정하며, `en-GB` 는 `United Kingdom`식 영어로 바꿉니다. `-` 이후의 Country codes는 optional이며, 축약형으로 `locale: "en"` 라고 써도 됩니다. `locale: "ko-KR"`이 잘 작동하는 것으로 봐서, `locale: "ko"`도 잘 작동할 것으로 보입니다. 컨츄리 코드는 [reference table](<https://msdn.microsoft.com/en-us/library/ee825488(v=cs.20).aspx>) 에서 확인 가능합니다.

locale의 설정에 따라 [**UI Text**]({{ "/blog/ui-text/" | relative_url }}) 의 스크립트들이 바뀌어 적용됩니다. 한글 번역은 구글 번역기 돌린것같길래 어느정도 다듬어서 새로 작성해 주었습니다.

###  사이트 타이틀

사이트 제목입니다. masthead와 같은 곳이나 `<title>` 태그에 사용됩니다.

_Example:_ `title: "My Awesome Site"`

_Example:_ `title_separator: "|"` 를 통해 Page | title 형식으로 `Sample Page | My Awesome Site` 와 같이 표시할 수도 있습니다.

**Note:** 타이틀이 너무 길 경우 masthead가 제대로 표시되지 않을 수 있으므로, 가능하면 제목은 간결하게 해주세요.
{: .notice--warning}
...
Consult the [JavaScript documentation]({{ "/blog/javascript/" | relative_url }}) for more information on working with theme scripts.
{: .notice--info}
...
Breadcrumb start link text and separator character can both be changed in the [UI Text data file]({{ "/blog/ui-text/" | relative_url }}).
...
To customise the separator between the page date and reading time (if both are enabled), edit `.page__meta-sep::before` in a [custom stylesheet]({{ "/blog/stylesheets/" | relative_url }}).
...
To customize the author sidebar, read the full [layout documentation]({{ "/blog/layouts/#author-profile" | relative_url }}).
...
You'll also need to include some Liquid and HTML to properly use the paginator, which you can find in the **Layouts** section under [Home Page]({{ "/blog/layouts/#home-page" | relative_url }}).
