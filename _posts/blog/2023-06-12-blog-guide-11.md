...
**Note:** More information on using this `_include` can be found under [**Helpers**]({{ "/blog/helpers/" | relative_url }}).
{: .notice--info}

## Taxonomy archives

If you have the luxury of using Jekyll plugins, the creation of category and tag archives is greatly simplified. Simply enable support for the [`jekyll-archives`](https://github.com/jekyll/jekyll-archives) plugin with a few `_config.yml` settings as noted in the [**Configuration**]({{ "/blog/configuration/#archive-settings" | relative_url }}) section and you're good to go.
...
A derivative archive page layout to be used as a simple home page. It is built to show a paginated list of recent posts based off of the [pagination settings]({{ "/blog/configuration/#paginate" | relative_url }}) in `_config.yml`.
...
**Note:** Jekyll can only paginate a single `index.html` file. If you'd like to paginate more pages (e.g. category indexes) you'll need the help of a custom plugin. For more pagination related settings check the [**Configuration**]({{ "/blog/configuration/#paginate" | relative_url }}) section.
{: .notice--info}
...
Feature blocks can be assigned and aligned to the `left`, `right`, or `center` with a sprinkling of YAML. For full details on how to use the `feature_row` helper check the [**Content**]({{ "/blog/helpers/" | relative_url }}) section or review a [sample splash page](https://github.com/{{ site.repository }}/blob/master/docs/_pages/splash-page.md).
...
**Note:** A page using the `layout: search` isn't compatible with the new [site search feature]({{ "/blog/configuration/#site-search" | relative_url }}) incorporated in the masthead.
{: .notice--warning}
...
**Please Note:** Paths for image headers, overlays, teasers, [galleries]({{ "/blog/helpers/#gallery" | relative_url }}), and [feature rows]({{ "/blog/helpers/#feature-row" | relative_url }}) have changed and require a full path. Instead of just `image: filename.jpg` you'll need to use the full path eg: `image: /assets/images/filename.jpg`. The preferred location is now `/assets/images/`, but can be placed elsewhere or external hosted. This all applies for image references in `_config.yml` and `author.yml` as well.
{: .notice--danger}
...
To assign more author links, add to the `author.links` array  in [`_config.yml`]({{ "/blog/configuration/" | relative_url }}) link so. Any of [Font Awesome's icons](https://fontawesome.com/v5/search) are available for use.
...
To change the color of the button use one of the built in [utility classes]({{ "/blog/utility-classes/#buttons" | relative_url }}). Or you can create a new button class to match whatever color you want.
...
Add the new `.btn--reddit` class to the `<a>` element from earlier, [compile `main.css`]({{ "/blog/stylesheets/" | relative_url }}) and away you go.
