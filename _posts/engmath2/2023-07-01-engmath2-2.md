...
$$\oint_C Pdx + Qdy = \oint_C \bold F(r)dr = \sum_i \int_{C_i} F(r)dr$$

(C를 따라 F를 선적분함.)

for i=1, $y=c$로 고정이고, $x=a\rightarrow b$ 이므로,

$\bold r(x) = [x,c]  \ (a\leq x\leq b)$

이면, $\bold r'(x) = [1,0]$

$$\int_{C_1}\bold F(\bold r) d\bold r  = \int_a^b [P(x,c), Q(x,c)]\cdot [1,0] dx = \int_a^bP(x, c)dx$$
...
(eg)  C : 단위원, $\bold F (x,y) = [y^2-7y, 2xy+2x]$

$$\oint_C \bold F(\bold r) d\bold r = \oint_C (y^2-7y)dx + (2xy+2x)dy = \int\int_D ((2x+2)-(2y-7))dxdy =9\int\int_D dxdy = 9\pi$$
...
$$ \bold r_v (u,v) = [-r\sin u \sin v,\  r \sin u \cos v, \ 0], \\ \bold r_u (u,v) = [r \cos u \cos v, \ r \cos u \sin v ,\  -r\sin u]$$

($\bold r_u $ : u가 변수, v를 고정시켰다는 의미)

From now on, consider only $\bold r_v \times \bold r_u \neq 0$

$\bold r_v \times \bold r_u$ 연속인 공면 $\Leftrightarrow $ S : orientable surface

### 곡면의 넓이

u,v 평면에서, $[u, u+\Delta u] , [v,v+\Delta v]$ 범위의 작은 사각형에 대한 실제 곡면의 넓이

실제 곡면에서는 $\bold r(u,v), \bold r(u+\Delta u, v), \bold r(u, v+\Delta v), \bold r(u+\Delta u, v+\Delta v)$
