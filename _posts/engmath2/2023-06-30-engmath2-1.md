...
$$\bf{r'}(t) = \lim\limits_{\Delta t\rightarrow 0} {\bf r(t+\Delta t)-\bf r(t) \over \Delta t } \\ = \bf v(t) = (\lim\limits_{\Delta t\rightarrow 0} {\bf x(t+\Delta t)-\bf x(t) \over \Delta t }, \cdots, \cdots)$$

$$\bf {v'}(t) = \bf r''(t) = \bf a(t) = \lim\limits_{\Delta t \rightarrow 0} {\bf v(t+\Delta t)-\bf v(t) \over \Delta t}$$

- $\bf a$ 의 방향 : 진행방향의 수직인 방향과 수평인 방향 성분으로 나뉨. (구심 가속도 생각해보기.)
    
    $$\bf a = \bf a_n + \bf a_t$$
    
    $$\bf a_t = proj_{\bf v} (\bf a)$$

    proj (정사영은 정의를 생각해보면, a 벡터 크기의 cos 만큼의 크기를 v방향 단위벡터에 곱한 것.)

    $$ proj_{\bf v} (\bf a) = \vert \bf a \vert \cos \gamma {\bf v \over \vert \bf v \vert} \\ \cos\gamma = {\bf a \cdot \bf v \over \vert \bf a \vert \vert \bf v \vert}$$

    $$ \bf a_t =  {\bf a \cdot \bf v \over {\vert \bf v \vert}^2} \bf v \\ \bf a_n = \bf a - \bf a_t = \bf a - {\bf a \cdot \bf v \over {\vert \bf v \vert}^2} \bf v$$

    추가적으로, $\vert \bf v \vert ^2 = \bf v \cdot \bf v $이므로, 내적을 이용하여 proj를 표현하는 것이 더 편할 수도 있다.
...
$$\bf v (t) = (-R\omega\sin{\omega t}, R\omega\cos{\omega t}, 0) \\
\bf a (t) - (R\omega^2\cos{\omega t}, -R\omega^2\sin{\omega t}, 0)$$

- 속도와 가속도의 크기

$$v=\vert \bf v(t) \vert = R\omega \\ a = \vert \bf a (t)\vert = R\omega ^2$$
...
$$ \lim\limits_{n\rightarrow \infty} \sum_{k=1}^n \vert \bf r(t_k)-\bf r(t_{k-1})\vert = \overgroup{AB}$$

- 중간값 정리에 의해 

$$ \bf r(t_k) - \bf r(t_{k-1}) = \bf r ' (c)(t_k - t_{k-1}) \\ t_{k-1} < c < t_k$$

$$ n \rightarrow \infty, t_k \approx c \\ \bf r(t_k) - \bf r(t_{k-1}) = \bf r' (t_k) (t_k-t_{k-1}) = \bf r' (t_k) \Delta t $$

$$ l = \lim\limits_{n\rightarrow \infty} \sum_{k=1}^n \vert \bf r' (t_k) (t_k - t_{k-1}) \vert $$

정적분의 정의에서 활용한 점을 이용하면, $t_k - t_{k-1} = {b-a\over n} $, 즉 등간격이므로,

$$  \lim\limits_{n\rightarrow \infty} \sum_{k=1}^n \vert \bf r' (t_k) ({b-a \over n})\vert \\= \int_a^b \vert \bf r'(t)\vert dt$$
...
$$\int_a^b \vert \bf r'(t) \vert dt $$

- $t=a$ (시점)부터 $[a, b]$ 구간 내의 임의의 $t$에 대해 호의 길이 또한 하나의 함수가 됨, 또한 구간 내에서 함수는 멈추는 점이 없다면 ($\vert \bf r'(t) \vert \neq 0$) 증가함수이므로 매개변수 t에 대해 일대일대응이며 역함수가 존재함을 알 수 있다. 

![](/assets/images/engmath2-1/4.jpg)


$$ s(t) : = \int _a ^t \vert \bf r'(\tau) \vert d\tau \ (a\leq t \leq b)$$

- 호의 길이가 하나의 함수가 됨 : arc length function

$$ {d(s(t))\over dt} = \vert r' (t) \vert  = \bf v(t)$$
...
$$ {d (\bf r (t) )\over  ds } = {d (\bf r (t))\over dt} \cdot {dt \over ds} = {d (\bf r (t))\over dt} \cdot {1 \over {ds \over dt} } = {\bf r'(t) \over v(t)} = {\bf r'(t) \over \vert \bf r'(t) \vert }$$
...
$$ \bf \kappa(s) = {d(\bf u(s)) \over ds} $$

$$ \bf \kappa(s) = {d^2 r \over ds^2}\ \vert \bf \kappa(s)\vert = \kappa (s) := Curvature$$  

$$ {d \over ds} ({\bf r'(t)\over \vert \bf r'(t)\vert}) = {d \over ds} ({\bf r'(t)\over \vert \bf r'(t)\vert}) \cdot {ds \over dt}= {\bf r''(t)\over {\vert \bf r'(t)\vert}^2 }$$

if $ \vert \bf r'(t) \vert = 1 $, $\bf \kappa = \bf r''(t)$ (acceration vetor)
...
$$ \{ f(\bf r(t)) \}' = {d(f(\bf r(t)))\over dt} = {\partial f(\bf r(t))\over \partial x}\cdot {dx \over dt} +{\partial f(\bf r(t))\over \partial y}\cdot {dy \over dt} +{\partial f(\bf r(t))\over \partial z}\cdot {dz \over dt} \\ = \nabla f(\bf r(t)) \cdot \bf r'(t)$$
...
level surface위의 점 P의 접평면 (tangent plane)에 대한 법벡터 $\bf n = \nabla f(P)$
...
$$ \bf E(P) = {(P-P_0) \over r} \cdot {kQ\over r^2} = k{Q\over r^3} (P-P_0) = k {Q (x-x_0, y-y_0, z-z_0) \over ((x-x_0)^2+(y-y_0)^2+(z-z_0)^2)^{3/2} }$$
...
$$ [v_1, v_2, v_3] = \bf v \rightarrow div\bf v = ({\partial v_1\over \partial x}+{\partial v_2\over \partial y}+{\partial v_3\over \partial z}) = \nabla \cdot \bf v$$
...
$${\Delta m \over \Delta t } \approx div(\rho \bf v)(P) \Delta V$$

$$ div(\rho \bf v) (P) \approx { {\Delta m \over \Delta t } \over \Delta V} $$

- 단위체적당 질량 변화율 (${\dot m \over \Delta V}$)

    $$ div (\rho \bf v) (P, t_0) = \lim\limits_{\Delta V \rightarrow 0} {\lim\limits_{\Delta t \rightarrow 0} {\Delta m \over \Delta t} \over \Delta V}$$
...
- 이를 div로 정리된 식에 대입 하면,
    ...
    $$ div(\rho \bf v) (P) = \lim\limits_{\Delta t \rightarrow 0} {\rho (P, t_0+\Delta t)-\rho (P,t_0) \over \Delta t} = -{\partial \rho (P, t_0)\over \partial t}$$

- Reynolds Transport Theorem (a.k.a. Mass conversion law) , simplified
    
    $$ div(\rho \bf v) + {\partial \rho \over \partial t} = 0 $$
...
$$ \bf v = (v_1, v_2, v_3) \rightarrow curl \bf v := \nabla \times \bf v \\ = ({\partial v_3 \over \partial y}-{\partial v_2 \over \partial z}, {\partial v_1 \over \partial z}-{\partial v_3 \over \partial x}, {\partial v_2 \over \partial x}-{\partial v_1 \over \partial y})$$

(eg) $\bf \omega = (0,0,\omega)$ 로 회전하는 경우를 고려해보자.

$$\bf r = (x,y,z) \\ \bf v = \bf \omega \times \bf r = [0,0,\omega] \times [x,y,z] $$

$$ curl \bf v = curl (\bf \omega \times \bf r) = curl ([-\omega y, \omega x, 0]) = [0,0,2\omega]$$

- 유체의 회전은 ${1 \over 2} \cdot curl \bf v$ 로 나타낼 수 있음을 알 수 있다.
